...
const ARCH = {
  nodes: {
    ...
    hitl_eval: {
      ...
      cx: 660, cy: 160,
      detail: {
        heading: 'HITL trigger evaluation',
        body: `After all BUAtlas invocations complete, the orchestrator evaluates 7 HITL triggers in priority order:\n\n1. priority_p0 — any BU head assigned P0 priority\n2. second_weak_from_gate_5 — two or more BUs rated WEAK (uncertain relevance)\n3. confidence_below_threshold — any gate decision below its policy minimum\n4. agent_escalate — any agent returned ESCALATE\n5. restricted_term — MLR-sensitive or commitment language in draft\n6. mlr_sensitive — scientific communications content flag\n7. dedupe_or_rate_limit — duplicate or rate-limit conflict\n\nIf any trigger fires, the pipeline immediately routes to AWAITING_HITL. PushPilot is skipped entirely.`,
        pills: ['7 triggers', 'ordered priority', 'pre-PushPilot'],
        accent: 'hook',
      },
    },
    pushpilot: {
    ...
    pre_deliver: {
      ...
      detail: {
        ...
        body: `The pre_deliver hook is the last policy gate before any notification leaves the system. It enforces:\n\nQuiet hours — if the current time falls in the BU's quiet window (e.g., 21:00–07:00 local time), SEND_NOW is overridden to HOLD_UNTIL(morning).\n\nChannel approval — only channels explicitly approved in the BU's channel policy may receive notifications.\n\nThis hook runs on the code side, not the agent side. It has access to the system clock and policy config — things an LLM can't reliably reason about. When it overrides PushPilot's preference, the divergence is recorded for audit.`,
        pills: ['quiet hours', 'channel policy', 'fail-closed', 'divergence audit'],
        accent: 'hook',
      },
    },
    terminal: {
  ...
  edges: [
    ...
    { from: 'buatlas',      to: 'hitl_eval',     label: 'briefs' },
    { from: 'hitl_eval',    to: 'pushpilot',     label: 'no trigger',  style: 'solid' },
    { from: 'hitl_eval',    to: 'terminal',      label: 'HITL',        style: 'dashed' },
...
export function initArchitecture() {
  if (_initialized) {
    // Already built — just replay entrance on re-show
    _runEntrance();
    return;
  }
  _initialized = true;
...
function _drawEdges() {
  ...
  ARCH.edges.forEach((edge, i) => {
    ...
    // Compute path: straight line from node edge to node edge
    const { x1, y1, x2, y2 } = _edgePoints(from, to, edge.shortcut);

    const path = _svgEl('path');
    const d = _routePath(x1, y1, x2, y2, from, to, edge.shortcut);
    ...
    // Edge label (skip audit edges — too noisy)
...
function _edgePoints(from, to) {
  // Simple: connect from right/left/top/bottom based on relative position
...
function _routePath(x1, y1, x2, y2, from, to, shortcut) {
  // Simple bezier curve
  const dx = x2 - x1;
  const dy = y2 - y1;
  const cx1 = x1 + dx * 0.5;
...
function _approxPathLen(d) {
  // Rough approximation: parse M and endpoint, compute straight-line distance * 1.3
...
function _runEntrance() {
  ...
  // Ordered entrance: nodes appear in pipeline order, edges draw after adjacent nodes appear
  const nodeOrder = [
    'ingest', 'signalscribe', 'post_ss', 'buatlas',
    'hitl_eval', 'pushpilot', 'pre_deliver', 'terminal', 'audit',
  ];
  ...
  // Edges draw after all nodes have appeared (delay = nodeOrder.length * 120 + 80)
  const edgesStart = delay + 100;
  edgeEls.forEach((e, i) => {
    const len = parseFloat(e.dataset.len || '200');
    const dur = 400 + i * 80;
    setTimeout(() => {
  ...
  // Show edge labels after edges finish
...
function _onKey(e) {
  // Only handle when architecture tab is visible
  const root = document.getElementById('arch-root');
